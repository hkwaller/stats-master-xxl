...
const TEAM_NAMES = {
  ...
  PHX: 'Phoenix Coyotes',
  QUE: 'Québec Nordiques',
  WIN: 'Winnipeg Jets (1979–96)',
  PIR: 'Pittsburgh Pirates',
  MMR: 'Montréal Maroons',
  STE: 'St. Louis Eagles',
  HAM: 'Hamilton Tigers',
  PHQ: 'Philadelphia Quakers',
  TAN: 'Toronto Arenas',
...
function resolveTeamNames(abbrevs) {
  ...
  return abbrevs
    .split(',')
    .map((a) => TEAM_NAMES[a.trim()] ?? a.trim())
    .join(', ')
}
...
async function main() {
  ...
  if (existsSync(CHECKPOINT)) {
    try {
      ...
      if (cp.allRecords?.length > 100) {
        ...
        console.log(
          `Resuming: ${Object.keys(completedSeasons).length} seasons done, ${allRecords.length} records\n`,
        )
      }
    } catch {
      /* start fresh */
  ...
  const activePlayers = new Set(
    allRecords.filter((r) => r.seasonId === 20252026).map((r) => r.playerId),
  )
