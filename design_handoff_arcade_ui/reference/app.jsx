...
const DIRECTIONS = [
  { id:'A', name:'Broadcast', tagline:'Arena scoreboard · LED dot-matrix · jumbotron energy', bg:'#07080c', fg:'#fff', bar:'#10131a' },
  { id:'B', name:'Collector', tagline:'Trading-card foils · halftone zine · warm paper',     bg:'#f4efe3', fg:'#141010', bar:'#141010' },
  { id:'C', name:'Arcade',    tagline:'Ice-rink arcade · playful chunk · mascot + streaks',   bg:'#eaf2ff', fg:'#0a1535', bar:'#0a1535' },
];
const SCREENS = [
...
function Frame({ label, children }) {
  return (
    <div style={{ display:'flex', flexDirection:'column' }}>
      <div style={{ font:'700 11px/1 "JetBrains Mono", monospace', letterSpacing:'0.22em', color:'#666', textTransform:'uppercase', marginBottom:10 }}>{label}</div>
...
function DirectionBlock({ dir }) {
  ...
  return (
    <div data-screen-label={`${dir.id} ${dir.name}`} style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:40 }}>
      <div className="direction-heading" style={{ background: dir.bar, color: dir.id==='B' ? '#f4efe3' : '#fff', width:1280 }}>
        <span className="eyebrow">Direction {dir.id}</span>
        <h2 style={{ color: dir.id==='A' ? '#ff2a44' : dir.id==='B' ? '#c8102e' : '#e32437' }}>{dir.name}.</h2>
        <p>{dir.tagline}</p>
      </div>
...
function App() {
  const [view, setView] = React.useState(() => {
    try { return JSON.parse(localStorage.getItem('sm-view')) || { dir:'A' }; } catch { return { dir:'A' }; }
  });
  const [editMode, setEditMode] = React.useState(false);

  React.useEffect(() => { localStorage.setItem('sm-view', JSON.stringify(view)); }, [view]);
  ...
  return (
    <div className="frame-host">
      {DIRECTIONS.map(d => <DirectionBlock key={d.id} dir={d}/>)}
    </div>
  );
}
