...
export function useJoinGame() {
  return useMutation(({ storage }, { id, name }: { id: string; name: string }) => {
...
export function useClaimBoss() {
  return useMutation(({ storage }, { playerId, token }: { playerId: string; token: string }) => {
    const game = getGame(storage)
    if (!game) return
...
export function useSaveSettings() {
  return useMutation(
    ({ storage }, { config, requesterId }: { config: GameSetupConfig; requesterId: string }) => {
      const game = getGame(storage)
      if (!game) return
      ...
      game.set(
        'careerRevealOrder',
        config.careerRevealOrder as unknown as import('@/types/game').CareerRevealOrder,
      )
      game.set('careerMinSeasons', config.careerMinSeasons)
      game.set('careerMaxReveals', config.careerMaxReveals)
      game.set(
        'hlComparisonField',
        config.hlComparisonField as unknown as import('@/types/game').HLComparisonField,
      )
...
export function useStartGame() {
  return useMutation(
    (
      ...
      game.set(
        'playerPowerups',
        playerPowerups as unknown as Record<string, Record<string, number>>,
      )
...
export function useNextQuestion() {
  return useMutation(
    ({ storage }, { requesterId, choices }: { requesterId: string; choices: string[] }) => {
      const game = getGame(storage)
      if (!game) return
...
export function useSubmitAnswer() {
  return useMutation(({ storage }, { playerId, answer }: { playerId: string; answer: string }) => {
    const game = getGame(storage)
    if (!game) return
    ...
    const existingAnsweredAt = (game.get('answeredAt') as unknown as Record<string, number>) || {}
    game.set('answeredAt', {
      ...existingAnsweredAt,
      [playerId]: elapsedMs,
...
export function useRevealAnswers() {
  return useMutation(({ storage }, requesterId: string) => {
    ...
    players.forEach((player, index) => {
      ...
      const usedDoubleDown =
        (playerPowerups[player.id]?.doubledown ?? POWERUP_INITIAL_CHARGES.doubledown) === 0
...
export function useRequestHint() {
  return useMutation(({ storage }, { hintType }: { hintType: string }) => {
    const game = getGame(storage)
    if (!game) return
...
export function useActivatePowerup() {
  return useMutation(
    ({ storage }, { playerId, powerupType }: { playerId: string; powerupType: PowerupType }) => {
      const game = getGame(storage)
      if (!game) return
      ...
      switch (powerupType) {
        ...
        case 'eliminate': {
          ...
          const existing =
            (game.get('playerEliminatedChoices') as unknown as Record<string, string[]>) || {}
...
export function useNextCareerRound() {
  return useMutation(({ storage }, { requesterId }: { requesterId: string }) => {
    const game = getGame(storage)
    if (!game) return
...
export function useBuzzIn() {
  return useMutation(({ storage }, { playerId }: { playerId: string }) => {
    const game = getGame(storage)
    if (!game) return
